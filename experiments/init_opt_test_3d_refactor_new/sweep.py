#!/usr/bin/env python
"""init_opt_test_3d_refactor_new sweep: BAYES-OPTIMAL model on H^3, curvature swept.
...
4 x 9 x 7 x 3 x 3 = 2268 runs, one SLURM job each.

Both proposals are `stratified_exp`: `Proposal.proposal` gates plain `exp` behind
`allowed_exp` (commit 40e02bd), which `hyper_proposal` never passes, so the
original `exp` grid no longer runs. The proposal is part of the run name, so these
cells cannot collide with the `exp` ones.
...
this project is the CONTROL for its trained twins init_test_3d_refactor_new and
init_test_log_vce_3d_refactor_new: at each curvature it says what
test_wnelbo_ref the exact posterior scores under the pinned stratified_exp(0.1)
reference proposal. Where that lands below H(p) the estimator is at fault, not
the model -- and curvature is expected to move it, because the heat-time ceiling
main_refactor.py clamps to is `_radial_t_max(3) * R^2` with R = 1/sqrt(-K),
i.e. 9.7 at K = -10, 97 at K = -1 and 9700 at K = -0.01, so the per-t loss
...
Two free correctness checks to read off the results:
  * test_wnelbo_ref / test_wce_ref come from the REFERENCE pass, pinned to
    stratified_exp(0.1) on its own RNG stream (salt=1) and independent of the
    swept loss geometry and rate. They must be IDENTICAL across all 21
    (geometry, rate) cells of a given (ps, K, seed).
