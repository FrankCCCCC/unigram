# init_refactor_9d — `init_test`'s grid on the general-d bridge, `hyper_dim = 9`
...
binary code path cannot reach. One of three sibling projects — `init_refactor_9d`,
`init_refactor_9d`, `init_refactor_16d` — that differ only in `hyper_dim`.

## Hypothesis

1. **The general-d path is a valid ELBO at d = 9.** `test_wnelbo_ref >= H(p)` in
   ...
   `wnelbo_ref = 1.6351 ± 0.0233` — on `H(p)` within 1.3 SE.
...
Fixed: `hyper_dim=9`, `lr=1e-3`, `gradient_clip_val=1.0`, `test_size=4e6`,
...
project name), so `experiments/report.py init_refactor_9d` works unchanged and the
cells pair 1:1 with `init_test` for a d = 2 vs d = 9 comparison.
...
Pilot on the 2080 Ti (the slowest node): ~0.04 s/step for d = 9 (the odd-d radial
sampler costs ~9 ms per `bridge()` call), 4M-sample test ≈ 80 s. Per
