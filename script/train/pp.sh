...
#
# The reference pass (which reports test_wnelbo_ref / test_ce_ref) deliberately
# uses the SAME proposal as the loss path, per experiments/init_test/setup.md.
...
OUTPUT_DIR="${OUTPUT_DIR:-output/init_test/ps-${PS}_lg-pp_q-${PROPOSAL}${EXP_RATE}_lr${LR}_st${MAX_STEPS}_s${SEED}}"
EXTRA="${EXTRA:-}"
...
exec "${PYTHON}" -u main.py \
    ...
    ref_proposal_type="${PROPOSAL}" \
    ref_proposal_exp_rate="${EXP_RATE}" \
    folder="${OUTPUT_DIR}" \
    ${EXTRA}
